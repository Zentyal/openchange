...
notif:
	...
	libmapiserver_process_queued_notifications(emsmdbp_ctx);

	/* Step 4. Process pending notifications
	 * Note: GetProps and GetRows are filled with flag NDR_REMAINING, which
	 * may hide the content of the following replies. */
	if ((idx > 0) &&
	    ((mapi_response->mapi_repl[idx].opnum == op_MAPI_QueryRows) ||
	     (mapi_response->mapi_repl[idx].opnum == op_MAPI_GetProps))) {
		goto end;
	} else {
		libmapiserver_process_notifications(mem_ctx, emsmdbp_ctx, mapi_response, &idx, &size);
	}
	...
	/* Step 5. Fill mapi_response structure */
					    ...
					    struct EcDoConnectEx *r)
{
	...
	emsmdbp_ctx = emsmdbp_init(dce_call->conn->dce_ctx->lp_ctx,
				   dcesrv_call_account_name(dce_call),
				   openchange_ldb_ctx, emsmdb_broker);
	if (!emsmdbp_ctx) {
