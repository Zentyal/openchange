						 ...
						 struct openchangedb_context **oc_ctx)
{
	...
	if (openchangedb_backend) {
		...
		mapi_status = openchangedb_mysql_initialize(mem_ctx,
						     openchangedb_backend,
						     oc_ctx);
	} else {
		DEBUG(0, ("Using default backend for openchangedb\n"));
		mapi_status = openchangedb_ldb_initialize(mem_ctx,
						   lpcfg_private_dir(lp_ctx),
						   oc_ctx);
	}

	if (mapi_status == MAPI_E_SUCCESS) {
		mapi_status = openchangedb_logger_initialize(mem_ctx, 0, ">>> ", *oc_ctx, oc_ctx);
	}
	return mapi_status;
}
